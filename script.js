/* ============================================================
   ...
   script.js
   All app logic: navigation, mood, journal, tasks,
   breathing exercise, profile, home stats
   ============================================================ */
...
function updateStreak() {
  ...
  if (profile.lastVisit !== today) {
    profile.streak = profile.lastVisit === yesterday
      ? (profile.streak || 0) + 1
      : 1;
...
function renderMoodHistory() {
  ...
  chartEl.innerHTML = recent.map((m, i) => {
    ...
    return `<div class="chart-bar-wrap">
      ...
      <span class="chart-label">${m.emoji}</span>
    </div>`;
  }).join('');
...
function renderJournals() {
  ...
  if (!journals.length) {
    el.innerHTML = '<div class="empty-state">Your diary is empty.<br>Write your first entry above! 📔</div>';
    return;
  }
...
let breathStep     = 0;
let breathSec      = BREATH_STEPS[0].dur;
let breathCycles   = 0;
...
const HT_MONTHS = ['January','February','March','April','May','June',
...
let htYear  = 2026;
let htMonth = new Date().getMonth() + 1; // default to current month
...
function htLoad(y, m) {
  ...
  if (y === 2026) {
    const seeded = HT_DEFAULT_HABITS.map((h, i) => ({
      id: Date.now() + i,
      name: h.name,
      goal: h.goal,
      marks: {}
...
function htSave(y, m, habits) {
  localStorage.setItem(htKey(y, m), JSON.stringify(habits));
}
...
function htSetMonth(m) {
  htMonth = parseInt(m, 10);
  htRender();
}
...
function htRender() {
  ...
  document.getElementById('ht-meta-badge').textContent =
    HT_MONTHS[htMonth - 1] + ' ' + htYear + ' · ' + totalDays + ' Days';
  ...
  for (let d = 1; d <= totalDays; d++) {
    ...
    hRow += `<th class="habit-col-day${wknd}">${d}<span>${HT_DAYS[dow]}</span></th>`;
  }
  ...
  if (!habits.length) {
    tbody.innerHTML = `<tr class="habit-empty-row"><td colspan="${totalDays + 5}">
      No habits yet — click <strong>Add New Habit</strong> above to get started! 🌱</td></tr>`;
    htUpdateSummary([]);
    return;
  }
...
function htUpdateSummary(habits) {
  ...
  const rating = overallPct > 95 ? '🏆🏆🏆 Excellent!'
               : overallPct > 80 ? '❤️❤️ Good Job!'
               : overallPct > 50 ? '⚡ Do Better!'
               : overallPct > 30 ? '👎 Low Effort'
               : habits.length   ? '⚠️ Keep Going!'
               :                   '—';
...
(function init() {
  ...
  initHomeMeta();
  updateHomeStats();
  // Set month select to current month, render tracker
  document.getElementById('ht-month-select').value = htMonth;
  htRender();
})();